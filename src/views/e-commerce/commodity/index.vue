              ...
              <template>
                <div class="type-category">
                  <el-form ref="categoryForm" label-width="80px">
                    ...
                    >
                      <el-input v-if="!item.values" v-model="item.valueName" placeholder="请输入内容"></el-input>
                      <el-select v-if="item.values" v-model="item.value" placeholder="请选择">
                        <el-option
        ...
        <div>
          ...
          <el-form :model="shoppingForm" :rules="rules">
              ...
              <div v-else>
                ...
                <div class="size-select">
                  <el-checkbox-group v-model="item.contents">
                    ...
                    >{{ itemsize.value }} <i v-if="itemsize.shopId === 1" class="el-icon-delete" @click.prevent="delDefaultValue(itemsize.id)"></i></el-checkbox>
                  </el-checkbox-group>
                </div>
              </div>
            </div>
            ...
            <el-form-item style="margin-left: 130px;width: 800px;">
              ...
              >
                <el-table-column v-for="(item,index) in columns" :label="item.specName" :key="index">
            ...
            <el-form-item class="upload-shopping-img">
              <template>
                <el-upload
                  ...
                  :limit= 6
                  :on-remove="handleShoppingImgRemove"
...
export default {
  ...
  watch: {
    list() {
      const result = this.list.reduce((result, arr) => {
        ...
        return result
          .map(item => arr.map(c => Object.assign({}, item, { [c.specId || '']: c })))
          .flat(1);
      }, []);
  ...
  methods: {
    ...
    async brandList() {
      const query = {};
      try {
        const ret = await api.brandList(query);
        if (ret.data.code == 200) {
          console.log(ret, "品牌列表获取");
    ...
    async setProductDetail(data) {
      try {
        let ret = await api.setProductDetail(data);
        console.log(ret);
        if (ret.data.code == 200) {
          console.log("发布成功");
        } else {
    ...
    async getProductDetail(query) {
      try {
        let ret = await api.getProductDetail(query);
        if (ret.data.code == 200) {
